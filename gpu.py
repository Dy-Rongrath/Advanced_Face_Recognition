...
def image_recognition(image_path, known_encodings, known_names, student_data):
    ...
    # Resize the image (optional, for consistency with video processing)
    image = cv2.resize(image, (0, 0), fx=FRAME_RESIZE_SCALE, fy=FRAME_RESIZE_SCALE)
...
def video_processing_thread(video_source, known_encodings, known_names, student_data, output_file, use_webcam=False):
    ...
    try:
        ...
        while True:
            ...
            gpu_frame_resized = cv2.cuda.resize(gpu_frame, (frame_width, frame_height))
            frame = gpu_frame_resized.download()
